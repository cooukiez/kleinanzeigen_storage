...
$anzeigen = $db->query("SELECT * FROM anzeigen WHERE sold = 0 AND short NOT LIKE ''");
...
<body>
    <h1 style="margin-bottom: 0">LKS</h1>
    <p style="margin-top: 0; margin-bottom: 5px">Lokale Kleinanzeigen Services</p>
    <button style="margin-top: 0" onclick="window.location.href='archive.php';">Go to Archive</button><br><br>
    <br>
    <form id="anzeige_form" action="upload.php" method="post" enctype="multipart/form-data">
        ...
        <select id="select_anzeige" name="select_anzeige" style="display: inline">
            ...
            if ($anzeigen->num_rows > 0) {
                while ($row = $anzeigen->fetch_assoc()) {
                    $option_style = ($row['ready'] == '1') ? 'background-color: pink;' : '';
                    echo "<option value='" . $row['id'] . "' style='" . $option_style . "'>" . $row['short'] . "</option>";
                }
            }
            ?>
        </select><button type="button" id="delete" style="display: none; margin-left: 15px">Delete Ad</button>
        <br><br>
        <hr><br>

        <p style="display: inline">Anzeige Id: </p>
        <p id="anzeige_id" style="display: inline"></p><br>
        <p style="display: inline">Status: </p>
        <p id="status" style="display: inline; color: red">Offline</p><br><br>
        ...
        <input type="text" id="title" name="title" required pattern=".{10,}"><br><br>
        ...
        <div id="image_display" name="image_display" style="height: 220px; white-space: nowrap; overflow-x: auto; width: 100%;"></div>
    ...
    if (isset($_SESSION['message'])) {
        ...
        echo '<hr><p>Status messages will show up here.</p><hr>';
    }
    ?>
    <script>
        <?php
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            var select_cache = localStorage.getItem('index_select_anzeige');
            var scroll_cache = localStorage.getItem('index_scroll');
            ...
            if (select_cache != null && anzeigen.some(obj => obj.id == select_cache && obj.sold == '0')) {
        ...
        function update_form(select) {
            localStorage.setItem('index_select_anzeige', select.value);
            var selected = select.value;
            var is_new = selected === 'new';
            document.getElementById('anzeige_form').action = is_new ? 'upload.php' : 'modify.php';
            document.getElementById('submit').value = is_new ? 'Submit' : 'Update';
            document.getElementById("delete").style.display = is_new ? "none" : "inline";
            document.getElementById("image_display").style.display = "none";

            var anzeige = is_new ? anzeigen[0] : anzeigen.find(obj => obj.id === selected);
            set_form_values(anzeige);
        ...
        function set_form_values(anzeige) {
            document.getElementById('anzeige_id').innerHTML = anzeige.id;
            var status = document.getElementById('status');
            var online_until = new Date(anzeige.online_until);
            var current_date = new Date();

            status.innerHTML = (online_until && online_until >= current_date) ? 'Online (until ' + anzeige.online_until + ')' : 'Offline';
            status.style.color = (online_until && online_until >= current_date) ? 'green' : 'red';
                ...
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        ...
                        imageData.sort(function(a, b) {
                            return a.img_position - b.img_position;
                        });
